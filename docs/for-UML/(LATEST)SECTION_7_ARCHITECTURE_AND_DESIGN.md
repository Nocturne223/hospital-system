...
At runtime, application code imports **`DatabaseManager` from `src/database/__init__.py`**: when `USE_MYSQL` in `src/config.py` is true, that symbol is **`MySQLDatabaseManager`**; otherwise it is the SQLite **`DatabaseManager`** from `db_manager.py`. Services always depend on this single exported type, matching the **Strategy** / composition-root pattern described below.
...
| **QueueService** | Add patient to queue, **`get_next_patient`** (next by priority then FIFO; internally marks served), **`serve_patient`** for a specific row, change priority, remove from queue; queue statistics and analytics; enforcement of specialization capacity. |
...
classDiagram
    ...
    ReportService --> SpecializationService : uses
...
flowchart TD
    ...
    B --> C{Specialization\nselected?}
    C -->|No| D([Show: Select a specialization])
    C -->|Yes| E([QueueService: get_next_patient])
    E --> F{Queue\nempty?}
    F -->|Yes| G([Return None / Show: No patients in queue])
    G --> H([End])
    F -->|No| I([Next entry by priority then FIFO;\ninternally serve_patient])
...
**Last updated:** March 22, 2026 (aligned with Beta 1.1 / Streamlit codebase)
